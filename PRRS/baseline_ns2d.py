...
def predict_ar(model, a_enc, out_norm, cfg, num_passes=1):
    ...
    for k in range(num_passes):
        ...
        pred_enc = torch.cat(preds, dim=-1)
...
def main(norm_pre=False, suffix=""):
    ...
    if method == "ensemble":
        ...
        for s in [0, 1, 2, 3, 4]:
            ...
            chk_path = os.path.join(RESULTS, 'ns2d_fno_seed'+str(s)+'.pt')
    ...
    if method in ["mc_dropout", "ensemble"]:
        scores_cal_raw = (pred_cal_k.std(dim=0) / (pred_cal_k.mean(dim=0).abs() + 1e-6)).mean(dim=(1,2,3,4)).numpy()
        scores_val_raw = (pred_val_k.std(dim=0) / (pred_val_k.mean(dim=0).abs() + 1e-6)).mean(dim=(1,2,3,4)).numpy()
        scores_ood_raw = (pred_ood_k.std(dim=0) / (pred_ood_k.mean(dim=0).abs() + 1e-6)).mean(dim=(1,2,3,4)).numpy()
    else:
    ...
    if norm_pre:
        if method in ["mc_dropout", "ensemble"]:
            scores_cal_n = scores_cal_raw
            scores_val_n = scores_val_raw
            scores_ood_n = scores_ood_raw
        else:
